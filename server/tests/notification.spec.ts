...
describe('GET /notifications/getAll', () => {
    ...
    it('should properly return existing notifications', async () => {
      ...
      const mockNotifications = [
        { user: 'user1', read: false, createdAt: currentDate },
        { user: 'user1', read: false, createdAt: new Date(currentDate.getTime() - 1000) }
      ];
      ...
      expect(response.body).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ user: 'user1', read: false })
        ])
      );
  ...
  describe('POST /notifications/markRead', () => {
    ...
    it('should successfully mark a notification as read', async () => {
      ...
      const mockFindByIdAndUpdate = jest.spyOn(NotificationModel, 'findByIdAndUpdate')
