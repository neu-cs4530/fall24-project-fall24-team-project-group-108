...
const getCorrespondencesByOrderSpy: jest.SpyInstance = jest.spyOn(util, 'getAllCorrespondences');
const fetchCorrespondenceByIdSpy: jest.SpyInstance = jest.spyOn(util, 'fetchCorrespondenceById');
const fetchAndIncrementCorrespondenceViewsByIdSpy: jest.SpyInstance = jest.spyOn(
  util,
...
describe('GET /getCorrespondence', () => {
  ...
  it('should return correspondences', async () => {
    ...
    const responseCorrespondences = (response.body as Correspondence[]).map(
      correspondence => correspondence._id,
    );

    expect(response.status).toBe(200);
    expect(responseCorrespondences).toEqual(
      CORRESPONDENCES.map(correspondence => correspondence._id),
    );
  });
...
describe('GET /getCorrespondenceById/:cid', () => {
  ...
  it('should return correspondence by given id', async () => {
    ...
    const response = await supertest(app).get(
      `/correspondence/getCorrespondenceById/65e9b58910afe6e94fc6e6ba`,
    );
...
describe('GET /getCorrespondenceByIdWithViews/:cid', () => {
  ...
  it('should return correspondence by given id with the correct amount of views', async () => {
    fetchAndIncrementCorrespondenceViewsByIdSpy.mockResolvedValueOnce({
      ...correspondence1,
      views: [...correspondence1.views, 'brother'],
    });

    const response = await supertest(app).get(
      `/correspondence/getCorrespondenceByIdWithViews/65e9b58910afe6e94fc6e6ba?username=brother`,
    );
