...
describe('POST /updateMessage', () => {
  ...
  it('should update a given messages messageText and isCodeStyle values', async () => {
    ...
    const mockCorrespondence = {... correspondence2, messages: [{...message1, messageText: 'new message text', isCodeStyle: false}]}
    ...
    // Making the request
    const response = await supertest(app).post('/message/updateMessage').send({mid: '65e9b58910afe6e94fc6e6aa', updatedMessageText: 'new message text', isCodeStyle: false});
  ...
  it('should return error 500 if updateMessageById throws an error', async () => {
    jest.spyOn(util, 'updateMessageById').mockRejectedValueOnce(new Error('Error when updating message'));
...
describe('POST /updateMessageViews', () => {
    ...
    it('should update a message views by adding the given user', async () => {
      ...
      // Making the request
      const response = await supertest(app).post('/message/updateMessageViews').send({mid: '65e9b58910afe6e94fc6e6aa', username: 'rclemens'});
    ...
    it('should return error 500 if updateMessageViewsById throws an error', async () => {
      jest.spyOn(util, 'updateMessageViewsById').mockRejectedValueOnce(new Error('Error when updating message'));
...
describe('POST /updateMessageEmojis', () => {
    ...
    it('should update a message emojis by setting the given emojis', async () => {
      ...
      // Making the request
      const response = await supertest(app).post('/message/updateMessageEmojis').send({mid: '65e9b58910afe6e94fc6e6aa', emojis: {'trichards': 'like', 'jromano': 'heart'}});
      const responseMessage = response.body as Message;
    ...
    it('should return error 500 if updateMessageEmojisById throws an error', async () => {
      jest.spyOn(util, 'updateMessageEmojisById').mockRejectedValueOnce(new Error('Error when updating message'));
